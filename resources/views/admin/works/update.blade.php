...
@section('title', 'Редагувати роботу')
@section('Stylesheets')
<style>
    .music, .video{
        display: none;
...
<section class="content-header">
    <div class="page-header">
        <h1>Роботи
            <small class="xs-hidden">тут можна редагувати роботу</small>
        </h1>
    </div>
</section>
@include('layouts._partials.feedback')
<section>
    <form action="{{ route('admin.works.postUpdate', ['id' => $work->id]) }}" class="form form-horizontal" method="post" enctype="multipart/form-data">
        <div class="form-group{{ $errors->has('title') ? ' has-error' : ''}}">
            <label for="title" class="control-label col-md-2">Заголовок</label>
            <div class="col-md-10">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <input class="form-control" id="title" name="title" placeholder="Заголовок" value="{{ $work->title }}">
                @if($errors->has('title'))
        ...
        <div class="form-group{{ $errors->has('typeId') ? ' has-error' : ''}}">
            ...
            <div class="col-md-10">
                <select class="form-control" id="typeId" name="typeId" value="{{ $work->typeId }}">
                    <option value="" selected disabled hidden>Виберіть тип</option>
                    <option value="100" {{ $work->typeId == 100 ? 'selected' : null}}>Музика</option>
                    <option value="200" {{ $work->typeId == 200 ? 'selected' : null }}>Відео</option>
                </select>
        ...
        <div class="form-group{{ $errors->has('author') ? ' has-error' : ''}}">
            ...
            <div class="col-md-10">
                <input class="form-control" id="author" name="author"  value="{{ $work->author }}">
        ...
        <div class="form-group{{ $errors->has('songname') ? ' has-error' : ''}}">
            ...
            <div class="col-md-10">
                <input class="form-control" id="songname" name="songname" value="{{ $work->songname }}">
                @if($errors->has('songname'))
                <span class="help-block">{{$errors->first('songname')}}</span>
                @endif
            </div>
        </div>
        @if(!empty($work->songUrl))
        <div class="col-md-10 col-md-offset-2 music">
            <audio src="{{ asset($work->songUrl) }}" controls="controls"></audio>
        </div>
        @endif
        ...
        <div class="form-group video{{ $errors->has('video') ? ' has-error' : ''}}">
            ...
            <div class="col-md-10">
                ...
                <input class="form-control" id="video" name="video" placeholder="Введіть ідентифікатор як показано на картинці" value="{{ $work->video }}">
                @if($errors->has('video'))
        ...
        <div class="form-group{{ $errors->has('text') ? ' has-error' : ''}}">
            ...
            <div class="col-md-10">
                <textarea class="form-control" id="text" name="text" placeholder="Напишіть щось...">{{ $work->text }}</textarea>
                @if($errors->has('text'))
